...
class GameBloc extends Bloc<GameEvent, GameState> {
  GameBloc()
    : super(GameState(tiles: [], score: 0, bestScore: 0, gameOver: false)) {
  ...
  Future<void> _applyMove(MoveResult result, Emitter<GameState> emit) async {
    ...
    if (newBest > state.bestScore)
      ScoreStorage.saveBestScore(state.gridSize, newBest);

    // Play merge sound for each tile that merged (same as popup animation pattern).
    final mergeCount = movedTiles.where((t) => t.isMerged).length;
    for (int i = 0; i < mergeCount; i++) {
      SoundService.playMerge();
    }
    ...
    emit(state.copyWith(tiles: spawnedTiles, gameOver: gameOver));
  }
