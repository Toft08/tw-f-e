...
class SoundService {
  // Separate players so merge and win sounds can overlap.
  ...
  static Future<void> playMerge() async {
    try {
      final bytes = _generateWav(
        segments: [const _Segment(frequency: 523.25, durationMs: 90, amplitude: 0.35)],
      );
      await _mergePlayer.play(BytesSource(bytes), volume: 0.6);
    } catch (_) {}
  }
  ...
  static Future<void> playWin() async {
    try {
      ...
      final bytes = _generateWav(segments: [
  ...
  static Uint8List _generateWav({required List<_Segment> segments}) {
    ...
    for (final seg in segments) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        final double value = sin(2 * pi * seg.frequency * t) * seg.amplitude * env;
    ...
    // RIFF header
    _writeString(buffer, o, 'RIFF'); o += 4;
    buffer.setUint32(o, 36 + dataSize, Endian.little); o += 4;
    _writeString(buffer, o, 'WAVE'); o += 4;

    // fmt chunk
    _writeString(buffer, o, 'fmt '); o += 4;
    buffer.setUint32(o, 16, Endian.little); o += 4;
    buffer.setUint16(o, 1, Endian.little); o += 2;          // PCM
    buffer.setUint16(o, numChannels, Endian.little); o += 2;
    buffer.setUint32(o, sampleRate, Endian.little); o += 4;
    buffer.setUint32(o, sampleRate * numChannels * bytesPerSample, Endian.little); o += 4;
    buffer.setUint16(o, numChannels * bytesPerSample, Endian.little); o += 2;
    buffer.setUint16(o, bitsPerSample, Endian.little); o += 2;

    // data chunk
    _writeString(buffer, o, 'data'); o += 4;
    buffer.setUint32(o, dataSize, Endian.little); o += 4;
